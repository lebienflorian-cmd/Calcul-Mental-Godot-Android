...
func _build_ui() -> void:
	...
	save_btn.custom_minimum_size = Vector2(0, 220)
	...
	back_btn.custom_minimum_size = Vector2(0, 220)
...
func _section(title: String) -> VBoxContainer:
	...
	header_btn.add_theme_font_size_override("font_size", ThemeManager.scaled_i(ThemeManager.FONT_MED * 2))
	header_btn.add_theme_stylebox_override("normal", ThemeManager.make_button_style(ThemeManager.SURFACE, 8))
	...
	header_btn.pressed.connect(func():
		...
		header_btn.text = ("▶  " if collapsed else "▼  ") + title
	)
	return body
